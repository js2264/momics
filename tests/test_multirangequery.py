...
def test_multirangequery_tracks(momics_path: str, bed1: str):
    ...
    q = MultiRangeQuery(mom, "I:990-1010").query_tracks()
...
def test_multirangequery_seq(momics_path: str):
    mom = momics.Momics(momics_path)
    q = MultiRangeQuery(mom, "I:0-10").query_sequence()
    assert len(q.seq) == 1
    assert q.seq["nucleotide"]["I:0-10"] == "ATCGATCGAT"

    q = MultiRangeQuery(mom, "I:9990-10000").query_sequence()
    assert len(q.seq) == 1
...
def test_to_json_npz(momics_path: str, temp_json_file: Path, temp_npz_file: Path):
    mom = momics.Momics(momics_path)
    q = MultiRangeQuery(mom, "I:0-10").query_sequence().query_tracks()
